  ...
  try {
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS db_canvas.canvases (
        ...
        canvas_type ENUM('board', 'diagram', 'mindmap', 'doc') NOT NULL DEFAULT 'board',
        data JSON NULL,
    ...
    if (canvasTypeCols.length === 0) {
      await conn.query("ALTER TABLE db_canvas.canvases ADD COLUMN canvas_type ENUM('board', 'diagram', 'mindmap', 'doc') NOT NULL DEFAULT 'board' AFTER unit");
      ...
      await conn.query("UPDATE db_canvas.canvases SET canvas_type = 'board' WHERE unit = 'board' OR canvas_type = 'pixel' OR canvas_type NOT IN ('board', 'diagram', 'mindmap', 'doc')");
      await conn.query("UPDATE db_canvas.canvases SET canvas_type = 'doc' WHERE unit = 'doc'");
      await conn.query("ALTER TABLE db_canvas.canvases MODIFY COLUMN canvas_type ENUM('board', 'diagram', 'mindmap', 'doc') NOT NULL DEFAULT 'board'");
    }
    ...
    logger.db.info('Tablas, columnas e índices de identidad, 2FA, suscripciones, compras, GeoIP, db_canvas, equipos, vistas, feedback IA, snapshots, notificaciones y soporte técnico verificadas exitosamente.');
  } catch (err) {
