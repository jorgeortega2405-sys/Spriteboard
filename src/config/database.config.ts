  ...
  try {
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS db_canvas.canvases (
        ...
        canvas_type ENUM('pixel', 'board', 'diagram', 'mindmap', 'doc') NOT NULL DEFAULT 'pixel',
        data JSON NULL,
    ...
    if (canvasTypeCols.length === 0) {
      await conn.query("ALTER TABLE db_canvas.canvases ADD COLUMN canvas_type ENUM('pixel', 'board', 'diagram', 'mindmap', 'doc') NOT NULL DEFAULT 'pixel' AFTER unit");
      ...
      await conn.query("UPDATE db_canvas.canvases SET canvas_type = 'pixel' WHERE canvas_type NOT IN ('pixel', 'board', 'diagram', 'mindmap', 'doc')");
      await conn.query("ALTER TABLE db_canvas.canvases MODIFY COLUMN canvas_type ENUM('pixel', 'board', 'diagram', 'mindmap', 'doc') NOT NULL DEFAULT 'pixel'");
    }
