  ...
  try {
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS db_canvas.canvases (
        ...
        canvas_type ENUM('board', 'doc') NOT NULL DEFAULT 'board',
        data JSON NULL,
    ...
    if (canvasTypeCols.length === 0) {
      await conn.query("ALTER TABLE db_canvas.canvases ADD COLUMN canvas_type ENUM('board', 'doc') NOT NULL DEFAULT 'board' AFTER unit");
      ...
      await conn.query("UPDATE db_canvas.canvases SET canvas_type = 'board' WHERE canvas_type != 'doc'");
      await conn.query("ALTER TABLE db_canvas.canvases MODIFY COLUMN canvas_type ENUM('board', 'doc') NOT NULL DEFAULT 'board'");
    }
