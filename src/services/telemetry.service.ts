...
class TelemetryService {
  private httpBuffer: HttpMetricInput[] = [];
  private eventBuffer: TelemetryEventInput[] = [];
  private systemBuffer: SystemMetricInput[] = [];
  private vitalsBuffer: WebVitalInput[] = [];

  private flushTimer: NodeJS.Timeout | null = null;
  private systemSampleTimer: NodeJS.Timeout | null = null;
  private activeRequestsCount = 0;
  private isFlushing = false;

  private readonly FLUSH_INTERVAL_MS = 5000;
  private readonly MAX_BUFFER_ITEMS = 100;
  private readonly MAX_RETAINED_BUFFER = 5000;
  ...
  public recordEvent(event: TelemetryEventInput): void {
  ...
  public recordSystemMetric(metric: SystemMetricInput): void {
  ...
  public recordWebVital(vital: WebVitalInput): void {
  ...
  public async getTelemetrySummary(): Promise<TelemetryStatsSummary> {
    ...
    try {
      ...
      for (const r of rows) {
        ...
        if (code >= 200 && code < 300) statusDistribution['2xx']++;
        else if (code >= 300 && code < 400) statusDistribution['3xx']++;
        else if (code >= 400 && code < 500) statusDistribution['4xx']++;
        else if (code >= 500) statusDistribution['5xx']++;
      }
