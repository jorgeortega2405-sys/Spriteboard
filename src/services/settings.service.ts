...
export interface UserPreferencesRecord extends RowDataPacket {
  ...
  theme: string;
  language: string;
  open_links_new_tab: boolean | number;
  telemetry: boolean | number;
  reduce_motion: boolean | number;
  high_contrast: boolean | number;
  extended_alerts: boolean | number;
  updated_at?: Date;
}
...
export async function logUserAudit(
  ...
  try {
    await pool.query<ResultSetHeader>(
      ...
      [userId, action, oldValue, newValue, ipAddress || null, userAgent ? userAgent.substring(0, 255) : null]
    );
...
export async function updateEmail(
  ...
  ip?: string | null,
  ua?: string | null
): Promise<{ success: boolean; email?: string; error?: string; status?: number }> {
