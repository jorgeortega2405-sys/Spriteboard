export const API_ROUTES = {
  config: '/api/config',
  csrfToken: '/api/csrf-token',
  health: '/api/health',
  avatarBase: '/api/avatar',
  avatar: (name: string) => `/api/avatar?name=${encodeURIComponent(name)}`,
  brandKits: {
    assets: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/assets`,
    assetById: (uuid: string, assetUuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/assets/${encodeURIComponent(assetUuid)}`,
    base: '/api/brand-kits',
    byId: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}`,
    charts: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/charts`,
    chartById: (uuid: string, chartUuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/charts/${encodeURIComponent(chartUuid)}`,
    colors: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/colors`,
    colorById: (uuid: string, colorUuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/colors/${encodeURIComponent(colorUuid)}`,
    default: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/default`,
    duplicate: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/duplicate`,
    fontById: (uuid: string, fontUuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/fonts/${encodeURIComponent(fontUuid)}`,
    fonts: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/fonts`,
    templates: (uuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/templates`,
    templateById: (uuid: string, templateUuid: string) => `/api/brand-kits/${encodeURIComponent(uuid)}/templates/${encodeURIComponent(templateUuid)}`,
  },
  canvases: {
    access: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/access`,
    base: '/api/canvases',
    byId: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}`,
    commentById: (uuid: string, commentUuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/comments/${encodeURIComponent(commentUuid)}`,
    comments: (uuid: string, frameIndex?: number) => frameIndex !== undefined ? `/api/canvases/${encodeURIComponent(uuid)}/comments?frameIndex=${encodeURIComponent(frameIndex)}` : `/api/canvases/${encodeURIComponent(uuid)}/comments`,
    delete: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}`,
    duplicate: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/duplicate`,
    heartbeatView: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/views/heartbeat`,
    members: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/members`,
    metrics: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/metrics`,
    move: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/move`,
    recordView: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/views`,
    removeMember: (uuid: string, userId: number) => `/api/canvases/${encodeURIComponent(uuid)}/members/${encodeURIComponent(userId)}`,
    removeTeam: (uuid: string, teamId: number) => `/api/canvases/${encodeURIComponent(uuid)}/teams/${encodeURIComponent(teamId)}`,
    resolveSlug: (slug: string) => `/api/canvases/resolve-slug/${encodeURIComponent(slug)}`,
    shared: '/api/canvases/shared',
    slug: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/slug`,
    snapshotById: (uuid: string, snapUuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/snapshots/${encodeURIComponent(snapUuid)}`,
    snapshotFork: (uuid: string, snapUuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/snapshots/${encodeURIComponent(snapUuid)}/fork`,
    snapshotRestore: (uuid: string, snapUuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/snapshots/${encodeURIComponent(snapUuid)}/restore`,
    snapshots: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/snapshots`,
    sync: '/api/canvases/sync',
    teams: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/teams`,
    token: (uuid: string) => `/api/canvases/${encodeURIComponent(uuid)}/token`,
  },
  folders: {
    base: '/api/folders',
    byId: (uuid: string) => `/api/folders/${encodeURIComponent(uuid)}`,
    canvases: (uuid: string) => `/api/folders/${encodeURIComponent(uuid)}/canvases`,
  },
  designer: {
    checkHandle: (handle: string) => `/api/designer/check-handle?handle=${encodeURIComponent(handle)}`,
    onboard: '/api/designer/onboard',
    onboardingStatus: '/api/designer/onboarding-status',
    poolSummary: '/api/designer/pool/summary',
    requestPayout: '/api/designer/payouts/request',
    stripeConnectLink: '/api/designer/stripe-connect/link',
    stripeConnectStatus: '/api/designer/stripe-connect/status',
  },
  designerApplications: {
    apply: '/api/designer-applications',
    myStatus: '/api/designer-applications/my-status',
  },
  favorites: {
    base: '/api/favorites',
    byType: (type: string) => `/api/favorites?type=${encodeURIComponent(type)}`,
    toggle: '/api/favorites/toggle',
  },
  teams: {
    base: '/api/teams',
    byId: (uuid: string) => `/api/teams/${encodeURIComponent(uuid)}`,
    canvases: (uuid: string) => `/api/teams/${encodeURIComponent(uuid)}/canvases`,
    members: (uuid: string) => `/api/teams/${encodeURIComponent(uuid)}/members`,
    removeMember: (uuid: string, userId: number) => `/api/teams/${encodeURIComponent(uuid)}/members/${encodeURIComponent(userId)}`,
  },

  enterprise: {
    checkDomainSso: (domain: string) => `/api/auth/sso/check-domain?domain=${encodeURIComponent(domain)}`,
    config: (type: string = 'business') => `/api/enterprise/config?type=${encodeURIComponent(type)}`,
    generateScimToken: '/api/enterprise/scim/token',
    revokeScimToken: '/api/enterprise/scim/token',
    samlMetadata: (uuid: string) => `/api/auth/sso/saml/metadata/${encodeURIComponent(uuid)}`,
    saveConfig: '/api/enterprise/config',
    ssoLogin: (domain: string) => `/api/auth/sso/login?domain=${encodeURIComponent(domain)}`,
  },
  trash: {
    base: '/api/trash',
    deletePermanent: (uuid: string) => `/api/trash/${encodeURIComponent(uuid)}`,
    empty: '/api/trash/empty',
    restore: (uuid: string) => `/api/trash/${encodeURIComponent(uuid)}/restore`,
  },
  templates: {
    base: '/api/templates',
    byId: (id: string) => `/api/templates/${encodeURIComponent(id)}`,
    deleteMyTemplate: (id: number | string) => `/api/templates/my/${encodeURIComponent(String(id))}`,
    myMetrics: '/api/templates/my/metrics',
    myTemplates: '/api/templates/my',
    publish: '/api/templates/publish',
    toggleVisibility: (id: number | string) => `/api/templates/my/${encodeURIComponent(String(id))}/visibility`,
  },
  search: (q: string) => `/api/search?q=${encodeURIComponent(q)}`,
  uploads: {
    base: '/api/uploads',
    byUuid: (uuid: string) => `/api/uploads/${encodeURIComponent(uuid)}`,
  },
  users: {
    banner: '/api/users/banner',
    follow: (username: string) => `/api/users/p/${encodeURIComponent(username)}/follow`,
    profile: (username: string) => `/api/users/p/${encodeURIComponent(username)}`,
    profileTemplates: (username: string) => `/api/users/p/${encodeURIComponent(username)}/templates`,
    search: (q: string) => `/api/users/search?q=${encodeURIComponent(q)}`,
  },
  notifications: {
    base: '/api/notifications',
    delete: (id: number) => `/api/notifications/${encodeURIComponent(id)}`,
    markAllRead: '/api/notifications/mark-all-read',
    markRead: (id: number) => `/api/notifications/${encodeURIComponent(id)}/read`,
  },
  chat: '/api/chat',
  chatFeedback: '/api/chat/feedback',
  ai: {
    board: '/api/ai/board',
    chat: '/api/chat',
    chatFeedback: '/api/chat/feedback',
    doc: '/api/ai/doc',
    mindmap: '/api/ai/mindmap',
    presentation: '/api/ai/presentation',
    quota: '/api/ai/quota',
    removeBackground: '/api/ai/remove-background',
  },
  support: {
    active: '/api/support/active',
    cancel: '/api/support/cancel',
    history: '/api/support/history',
    historyTicket: (ticketId: number) => `/api/support/history/${encodeURIComponent(ticketId)}`,
    message: '/api/support/message',
    messages: (ticketId: number) => `/api/support/messages/${encodeURIComponent(ticketId)}`,
    rate: '/api/support/rate',
    request: '/api/support/request',
  },
  auth: {
    forgotPassword: '/api/forgot-password',
    google: '/api/auth/google',
    googleCallback: '/api/auth/google/callback',
    googleLink: '/api/auth/google/link',
    googleVerify: '/api/auth/google/verify',
    login: '/api/login',
    login2fa: '/api/login/verify-2fa',
    logout: '/api/logout',
    logoutAll: '/api/auth/logout-all',
    me: '/api/me',
    registerResendCode: '/api/register/resend-code',
    registerSendCode: '/api/register/send-code',
    registerStage1: '/api/register/stage1-validate',
    registerVerifyCode: '/api/register/verify-code',
    resetPassword: '/api/reset-password',
    resetPasswordValidate: (token: string) => `/api/reset-password/validate?token=${encodeURIComponent(token)}`,
    switchAccount: '/api/auth/switch-account',
  },
  settings: {
    accountDelete: '/api/settings/account/delete',
    avatar: '/api/settings/avatar',
    email: '/api/settings/email',
    emailRequestCode: '/api/settings/email/request-code',
    emailVerifyCode: '/api/settings/email/verify-code',
    googleUnlink: '/api/settings/google/unlink',
    handle: '/api/settings/handle',
    password: '/api/settings/password',
    passwordStatus: '/api/settings/password/status',
    passwordVerify: '/api/settings/password/verify',
    preferences: '/api/settings/preferences',
    profileDetails: '/api/settings/profile-details',
    publicProfile: '/api/settings/public-profile',
    twoFactorDisable: '/api/settings/2fa/disable',
    twoFactorEnable: '/api/settings/2fa/enable',
    twoFactorGenerate: '/api/settings/2fa/generate',
    twoFactorStatus: '/api/settings/2fa/status',
    username: '/api/settings/username',
  },
  subscriptions: {
    autoRenewal: '/api/subscriptions/auto-renewal',
    base: '/api/subscriptions',
    cancelImmediate: '/api/subscriptions/cancel-immediate',
    checkout: '/api/subscriptions/checkout',
    details: '/api/subscriptions/details',
    history: '/api/subscriptions/history',
    paymentMethodDefault: (pmId: string) => `/api/subscriptions/payment-methods/${encodeURIComponent(pmId)}/default`,
    paymentMethodDelete: (pmId: string) => `/api/subscriptions/payment-methods/${encodeURIComponent(pmId)}`,
    paymentMethods: '/api/subscriptions/payment-methods',
    setupIntent: '/api/subscriptions/setup-intent',
    storage: '/api/subscriptions/storage',
    verifySession: (sessionId: string) => `/api/subscriptions/verify-session?session_id=${encodeURIComponent(sessionId)}`,
  },
  telemetry: {
    events: '/api/telemetry/events',
    stats: '/api/telemetry/stats',
    vitals: '/api/telemetry/vitals',
  },
} as const;

export default API_ROUTES;
