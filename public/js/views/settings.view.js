...
export async function createYourAccountView() {
  ...
  const refreshAvatarsInDom = (url) => {
    ...
    const topBarAvatar = document.querySelector(
  ...
  // Preferencias de Idioma y Enlaces
...
function showGoogleOrPasswordStep(status, onPasswordUpdated) {
  ...
  const currentPassInput = modal.body.querySelector(
    '[data-ref="modal-input-current-password"]'
  );
  const togglePassBtn = modal.body.querySelector(
...
function showStep2NewPassword(modal, onPasswordUpdated) {
  ...
  const inputConfirmPass = modal.body.querySelector(
    '[data-ref="modal-input-confirm-new-password"]'
  );
  const toggleNewPass = modal.body.querySelector(
    '[data-ref="toggle-modal-new-password"]'
  );
  const toggleConfirmPass = modal.body.querySelector(
...
export async function createBillingView() {
  ...
  const accordionHeaderSub = container.querySelector(
  ...
  const accordionHeaderPm = container.querySelector(
  ...
  const renderSubscriptionPlan = (info) => {
    ...
    const planStatusBadge = container.querySelector(
    ...
    const btnToggleRenewal = container.querySelector(
  ...
  const handleToggleAutoRenewal = (cancelAtPeriodEnd, dateFormatted = '') => {
    if (cancelAtPeriodEnd) {
      openModal({
        title:
          t('settings.billing.modal_cancel_renewal_title') ||
          '¿Cancelar renovación automática?',
        description:
          t('settings.billing.modal_cancel_renewal_desc', { date: dateFormatted }) ||
  ...
  const btnCancelImmediate = container.querySelector(
  ...
  btnCancelImmediate?.addEventListener('click', (e) => {
    ...
    openModal({
      title:
        t('settings.billing.modal_cancel_immediate_title') ||
        '¿Cancelar tu suscripción ahora?',
      description:
        t('settings.billing.modal_cancel_immediate_desc') ||
  ...
  const loadPaymentMethods = async () => {
    ...
    pms.forEach((pm, idx) => {
      ...
      btnDelete?.addEventListener('click', (e) => {
        ...
        openModal({
          title:
            t('settings.billing.modal_delete_card_title') || '¿Eliminar método de pago?',
          description:
            t('settings.billing.modal_delete_card_desc', {
              brand: brandUpper,
              last4: pm.last4,
            }) ||
...
export async function createPurchasesView() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (
      searchToolbar &&
      !searchToolbar.contains(e.target) &&
      btnToggleSearch &&
...
export async function createAccessibilityView() {
  ...
  const toggleExtendedAlerts = container.querySelector(
    ...
    if (prefRes.ok) {
      ...
      if (prefs) {
        ...
        if (prefs.theme && themeIcons[prefs.theme]) {
          ...
          if (themeDropdown) {
            const activeItem = themeDropdown.querySelector(
